...
var index$1 = (styles) => [plugin, styles || theme];
// #region Theme
...
const plugin = ViewPlugin.fromClass(class HideHeaderMarkPlugin {
    constructor(view) {
    ...
    for (const { from, to } of view.visibleRanges) {
        syntaxTree(view.state).iterate({
            enter: ({ type, from, to }) => {
                ...
                const cursorOverlaps = ranges.some(({ from: rangeFrom, to: rangeTo }) => rangeFrom <= line.to && rangeTo >= line.from);
                if (cursorOverlaps)
                    return;
...
export { index$1 as bold, index as capital };
